...
public class MinPriorityQueue<T extends Comparable> {
    ...
    private void rise(int index) {
        //通过迭代，不断地比较当前节点和其父节点值的大小; 如果父节点值比当前节点小，则交换位置(顶堆)
        while (index > 1) {
            //比较当前节点和其父节点，此处区别于大顶堆
    ...
    private void sink(int index) {
        //通过循环比较当前节点index和其左子节点(2k)、右子节点(2k+1)中较小值; 如果当前节点大，需要交换位
        while (2 * index <= N) {
            //获取当前节点中较小的孩子节点
            int min = 0; //记录较小节点处的索引
            /*当前节点有左右孩子*/
            if (2 * index + 1 <= N) {
